...
void Charge_DarknessEffect::Init()
{
	effects.clear();
}

void Charge_DarknessEffect::Update()
{
...
void Charge_DarknessEffect::Draw()
{
...
void Charge_DarknessEffect::LoadTexture()
{
    texture.Load("Texture/EffectTexture/Charge_Darkness_v1_A_Burst_Only_With_Darkness_spritesheet.png");
}

void Charge_DarknessEffect::Release()
{
    effects.clear();
    texture.Release();
}

void Charge_DarknessEffect::SetEffect(float posX, float posY, bool isActive)
{
    // 同じ位置に既存のエフェクトがないか確認
    for (const auto& effect : effects) {
