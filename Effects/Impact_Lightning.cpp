...
void Impact_LightningEffect::Init()
{
    effects.clear();
}

void Impact_LightningEffect::Update()
{
...
void Impact_LightningEffect::Draw()
{
...
void Impact_LightningEffect::LoadTexture()
{
    texture.Load("Texture/EffectTexture/Impact_Lightning_Lv3_spritesheet.png");
}

void Impact_LightningEffect::Release()
{
    effects.clear();
    texture.Release();
}

void Impact_LightningEffect::SetEffect(float posX, float posY, bool isActive)
{
    // 同じ位置に既存のエフェクトがないか確認
    for (const auto& effect : effects) {
