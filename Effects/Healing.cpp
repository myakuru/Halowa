...
void HealingEffect::Init()
{
    effects.clear();
}

void HealingEffect::Update()
{
...
void HealingEffect::Draw()
{
    for (const auto& effect : effects) {
        ...
        DirectX::SimpleMath::Color color = { 1.0f, 1.0f, 1.0f, 0.5f };
        SHADER.m_spriteShader.DrawTex(&texture, 0, 0, &srcRect, &color);
    }
}

void HealingEffect::LoadTexture()
{
    texture.Load("Texture/EffectTexture/Healing");
}

void HealingEffect::Release()
{
    effects.clear();
    texture.Release();
}

void HealingEffect::SetEffect(float posX, float posY, bool isActive)
{
    // 同じ位置に既存のエフェクトがないか確認
    for (const auto& effect : effects) {
